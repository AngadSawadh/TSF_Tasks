...
LABELS = open(class_names_path).read().strip().split("\n") #list of size num_class_names
top_points,rect_points,top_view_img = calibrator(r"G:\TSF_Tasks\Social Distancing Detector\samples\calibratecam.jpg")
...
while True:
    ...
    distances = dist.cdist(dist_points,dist_points,metric="euclidean")
    ...
    for (i,point_top) in enumerate(dist_points):
        px,py = point_top
        color = (0,255,0)
        if i in violate:
            color = (0,0,255)
    ...
    final = np.concatenate((frame,top_view),axis=1)
    text = "Total Violations:{}".format(len(violate))
    cv.putText(final,text,(0,70),cv.FONT_HERSHEY_SIMPLEX,0.85,(255,0,0),2)
    ...
    if args["output"] != "" and writer is None:
        ...
        writer = cv.VideoWriter(args["output"], fourcc, 25,(final.shape[1], final.shape[0]), True)
